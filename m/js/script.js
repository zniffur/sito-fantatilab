...
function LinkUtili() {
	...
	for (var i=1; i<=10; i++) {
		if (eval("Link"+i+"url")!="") {
			document.write ("<p><a href='../js/"+eval("Link"+i+"url")+"' target='"+LinkTarget+"'>"+eval("Link"+i)+"</a></p>")
		}
	}
}
...
// Crea la tabella con i link specificati nella generazione del sito
	...
	{	if (eval("Link"+j+"url") != "") // se è presente il collegamento Link'j'url diverso da ""
		{	document.write("<tr><td>") // apre riga e cella
			// collegamento
			document.write("<a href='../js/" + eval("Link"+j+"url") + "' target='" + LinkTarget + "'>" + eval("Link"+j) + "</a>")
