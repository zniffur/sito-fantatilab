...
<SCRIPT language=javascript type=text/javascript>
      ...
      <script src="js/fcmQueryFunzioni.js" type="text/javascript"></script>
  <script src="../js/fcmQueryDati.js" type="text/javascript"></script>
  <script language="javascript" type="text/javascript">
	cQ = JSQueryString("Q")
	if (CaricaDefault=="si") {
	...
	if (CaricaDefault) {
		if (cQ==null||cQ=="") cQ=arrQuery[1].ID
	}
	if (cQ!=null && cQ!="") document.write("<script src='../js/fcmQueryQ"+cQ+".js' type='text/javascript'></scr" + "ipt>") </script>
...
if (CaricaDefault) {
	...
	GeneraIntestazioneQuery(cQ)
//if (cQ!="" && cQ!=null) GeneraQuery(arrQuery[cQ].Nome)
if (cQ!="" && cQ!=null) {
	for (i=1; i<arrQuery.length;i++) {
		if (arrQuery[i].ID == cQ) GeneraQuery(arrQuery[i].Nome)
	}
}
document.close()
Stato("Query")
